...
- Provide immediate server-side revocation and client-side session wipe on logout or device loss.
...
- Version the protocol from the first frame: `hello {version, client, capabilities}`.
...
- Do not buffer sensitive output indefinitely for offline mobile clients.
