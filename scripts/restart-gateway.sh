...
: >"$LOG"
log "restart requested (host=$HOST port=$PORT shell=$SHELL_CMD token_file=$TOKEN_FILE)"
...
if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet server-console.service; then
  ...
  if systemctl restart server-console.service >>"$LOG" 2>&1; then
    for _ in $(seq 1 50); do
      if command -v ss >/dev/null 2>&1 && ss -ltn "sport = :$PORT" 2>/dev/null | grep -q "$HOST:$PORT"; then
        log "OK: gateway is listening on $HOST:$PORT"
        pgrep -af 'server_console_gateway.py' >>"$LOG" 2>&1 || true
        exit 0
      fi
      sleep 0.2
    done
    log "ERROR: service restarted but $HOST:$PORT is not listening"
    exit 1
  fi
...
BIND_HOST="$HOST" PORT="$PORT" SHELL_CMD="$SHELL_CMD" TOKEN_FILE="$TOKEN_FILE" \
