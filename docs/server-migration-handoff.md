...
3. WebSocket 协议使用 JSON 文本消息，终端字节用 base64。
4. 前端已支持：
   - WebSocket URL 输入。
...
```bash
python3 gateway/server_console_gateway.py --host 127.0.0.1 --port 8765 --token-file .server-console-token --mode pty
```
